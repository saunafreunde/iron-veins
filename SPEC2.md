...
| M23 | **v34** — `startYear`/`endless`/Klima-Set-Wahl, Mapgen-Knobs — **B1 eingeloest: der EINE Z5-Bump gehoert den zwei Ära-Regeln aus E-15 und ist auf genau ZWEI Felder verwendet, `startYear` (Kalenderjahr von Tick 0, Band 1850..1950) und `endless` (ob der MAX_TICK-Stopp ueberhaupt gilt). Beide UNBEDINGT gehasht, auf den Bedingungen von `editorMode`: keine hat zur Laufzeit einen abwesenden Zustand, also waere ein bedingter Hash Simulationsverhalten ausserhalb des Digests. Migration v33→v34 = 1950 + gebunden, weil das die aufgezeichnete Tatsache jeder je gespeicherten Welt ist; Pin `f1349c9b9c922981` → `1fa54bf95cc82b40` und Korpus `d3c2c16e6d8bf6e1` → `3b797322b43b717c` nach D-137/D-130 neu aufgenommen. Die ~60 Vor-1950-Specs brauchen KEINE Migration (eine Spec-Id ist eine Zahl in einem Konsist) und sind bis 1949 geschlossen, was jedes 19.4-Band per Konstruktion unberuehrt laesst — gemessen gegen einen Worktree bei `73e4c29`: 144 Zeilen, leerer Diff (D-245)**; **B2 (vier Klima-Oekonomien) loest KEINEN Bump ein und braucht keinen: kein Save-Feld, keine Migration, kein Snapshot-Byte, kein Protokollfeld — die Klima-Regel `climate` ist seit M1 gespeichert und gehasht, und alle vier neuen Tabellen (Industrieset, Fahrzeugmaske, Namensset, Stadt-Architektur) sind reine Funktionen von ihr. Kanonischer Pin `1fa54bf95cc82b40`, Korpus `3b797322b43b717c`, alle acht Szenarien hash-identisch samt Welt-Claims-, Briefing-Zahlen- und Ortsnamen-Audit, Szenario 5 auf 1.022.084 / 1.802.165 / 2.153.604 EUR, Punktzahl 5.889, Netzdesign 3,75, Winter 4,36 % — **nachgeprueft durch Ausfuehren**. Das temperierte Set IST der Katalog (Gewichtstabelle, Summe und jede Ziehung bit-identisch), weshalb sich kein 19.4-Band bewegt hat; bewegt haben sich die drei nicht-temperierten Szenarien in NAMEN und Industriezahl (2→5 / 7→9 / 10→11) samt genau EINER Briefing-Zahl. Haupt-Bundle 1.031.963 → **1.034.981 B gegen unveraenderte 1.040.000 B, Restluft 5.019 B** (D-246)**; **B3 (Generator-Presets und Regler) loest ebenfalls KEINEN Bump ein und ERWEITERT v34: das Weltregel-Record `mapgen` (Preset + fuenf Reglerschritte) wird gespeichert, unbedingt gehasht und von der bestehenden Migration v33→v34 eingetragen — ein Bump je Meilenstein, nicht ein Migrations-Edit (D-181-Muster). Sechs neue Hash-Woerter bewegen jeden Welt-Digest einmal, also nach D-137/D-130 neu aufgenommen: Pin `1fa54bf95cc82b40` → **`b7e632a7124e67ce`**, Korpus `3b797322b43b717c` → **`a00868b9911f12d6`** samt neu geschriebener `v34-played.ironsave` (eine v34-Datei aus einem Zwischenstand dieses Meilensteins fehlt `mapgen` und laedt nicht mehr — der ehrlich genannte Preis des Erweiterns), Soak-Fixture `1f1ac33ffed6afe9` → **`64fec78d6bf0cd5e`** bei unveraenderten 35 Kommandos (seit B1 rot, hier geschlossen). Der neutrale Boden hat sich NICHT bewegt — dreifach belegt: byte-identische Karten durch drei Tueren, drei byte-identisch neu aufgezeichnete 1024er-Benchmarks, und jedes 19.4-Band auf den Cent (Szenario 5 1.022.084 / 1.802.165 / 2.153.604 EUR, Punktzahl 5.889, Netzdesign 3,75, Winter 4,36 %). Bewegt hat sich genau die 2048er Karte: die `TOWN_COUNT_MAX`-Klemme skaliert jetzt mit der Flaeche (140 → 559 Staedte, 534 → 653 Industrien), weshalb `archipelago.json` neu aufgezeichnet wurde. Haupt-Bundle 1.034.981 → **1.039.413 B**, Budget mit dieser Messung 1.040.000 → **1.048.000 B** (D-247)**; **B4 (Ären-Optik, Klima-Matrix, Abschluss) löst KEINEN Bump ein, verbraucht keinen und erweitert keinen: kein Save-Feld, keine Migration, kein Snapshot-Byte, kein Protokollfeld, keine Atlas-Zelle, keine i18n-Zeile, keine Allowlist-Zeile. Die Ära ist eine reine Funktion des veröffentlichten Jahres (`SnapshotI32.Year`, seit je im Snapshot), also gibt es nichts zu speichern und nichts, was veralten könnte — die Abnahmebedingung „1950 gestartet, 2049 geladen zeigt 2049“ ist damit eine Konstruktion und keine Zusicherung. Kanonischer Pin `b7e632a7124e67ce`, Korpus `a00868b9911f12d6`, Soak-Fixture `64fec78d6bf0cd5e` unbewegt, alle 19.4-Bänder auf den Cent (Szenario 2 249.980 EUR / Rückfluss Jahr 6, Holzkette 159.516 EUR, Punktzahl 5.889, Netzdesign 3,75, Winter −4,36 %, Szenario 5 1.022.084 / 1.802.165 / 2.153.604 EUR) — **nachgeprüft durch Ausführen** (Determinismus 9 Dateien/44 Tests grün, volle Balance-Suite 14 Dateien/131 Tests grün). Zwei neue Konstanten in `constants.ts` und nur dort (`TOWN_ERA_START_YEARS`, `SEASON_CLIMATE_HEAT`, beide mit Einheit und Herkunft annotiert). Haupt-Bundle 1.039.413 → **1.041.208 B** gegen unveränderte 1.048.000 B, **Restluft 6.792 B** (D-248)** | — **B2: null Byte (D-246)**; **B4: null Byte — die Ära liest das Jahr, das der Snapshot seit M0 führt (D-248)** | +0,05 ms (Ären-Kurven im Monats-Hook) — **B1 gemessen: 1,444 / 3,082 ms p50/p99 gegen die M10-Grundlinie 1,45 / 3,26 auf der Referenzflotte; B1 legt keinen Tick-Pfad an (der Katalog ist Daten, `epochYears` ist ein Getter an bestehenden Naehten, die abgeleiteten Schichten laufen einmal bei der Genese). Mapgen 1024² 2.801 ms gegen 8.000 (D-245)**; **B2 gemessen: Tick p50 **1,464** / p99 **2,934** ms auf der 1500-Fahrzeug-Referenzflotte (max 27,827 ms ueber 6.500 Ticks) gegen die M10-Grundlinie 1,45 / 3,26 — p99 DARUNTER, p50 0,014 ms darueber und damit weit im dokumentierten Laufrauschen; B2 legt keinen Tick-Pfad an (die Klima-Sets sind Mapgen und der Jahres-Hook, das Verfuegbarkeitstor laeuft zur Kommando-Zeit) (D-246)**; **B3 gemessen: Tick p50 **1,516** / p99 **3,126** ms auf derselben Referenzflotte gegen 1,45 / 3,26 — p99 DARUNTER, p50 0,066 ms darueber und damit im dokumentierten Laufrauschen; B3 legt ebenfalls keinen Tick-Pfad an (Preset und Regler laufen einmal bei der Genese). Mapgen-Band je Groessenstufe NEU und gemessen: 256² 223 ms / 512² 695 ms / 1024² 3.010 ms / 2048² 12.003 ms gegen 900 / 2.000 / 8.000 / 30.000 ms, und jeder der fuenf Presets erzeugt eine 1024er-Karte in 2.734–2.826 ms gegen 8.000 (D-247)**; **B4 legt keinen Tick-Pfad an und misst deshalb keinen: die Ära ist Render, die Matrix ist ein Test, und die EINE Sim-Zeile des Bundles (`heatSeasonFactor`) ersetzt einen Tabellenzugriff durch einen Funktionsaufruf EINMAL je Spieltag, aus der Regionenschleife herausgezogen — dieselbe Naht, an der D-204 das Frost-Tor gemessen hat. Nachgemessen wurde trotzdem: Tick p50 **1,638** / p99 **3,557** ms auf der 1500-Fahrzeug-Referenzflotte (max 21,140 ms über 6.500 Ticks) gegen die M10-Grundlinie 1,45 / 3,26 — 0,30 ms über der Grundlinie und damit im dokumentierten ±0,7-ms-Laufrauschen, auf einer Maschine, die unmittelbar davor die volle Balance-Suite gefahren hat; die Referenzflotte fährt mit Wetter „aus“, wo die geänderte Zeile nie erreicht wird. Gemessen ist stattdessen vor allem die CI-Minute, das fünfte Gemeingut: volle Balance-Suite **132,9 s → 162,3 s** (D-248)** | ~150 Zellen (Ären/Klima, Seite 2) — **B1 bucht NULL: die sechzig Vor-1950-Specs bleiben prozedural (E-14 nennt die fruehen Ären beim Namen), weil 60 Modelle × 8 Facings 480 Zellen waeren — gegen ~150 gebuchte und eine seit D-173 volle Detail-Seite (Fehlerkatalog 40, D-245)**; **B2 bucht ebenfalls NULL: die vier Stadt-Architekturen sind ein Repaint der sechs bestehenden prozeduralen Zellen ueber die M18-Regenerationsnaht (D-202). Offen und benannt: die gebackenen `building:`-Zellen bleiben klimablind — eine Klima-Achse im Bake ist das Vierfache der Gebaeudezellen und gehoert in das Kunstbundle, das die ~150 besitzt (D-246)**; **B4 bucht ebenfalls NULL: die drei Ären sind ein Repaint derselben sechs prozeduralen Zellen über die M18-Naht (D-202), gehalten unter D-206s 2,00-Kachelhöhen-Regel (gemessen 1,27 / 1,34 / 1,43). Bezahlt wird es damit, dass die gebackenen `building:`-Zellen ab der Platten-Ära abgelehnt werden und die Kachel prozedural zeichnet (E-14s Lückenfüller-Regel) — die ~150 Zellen bleiben ungebucht und gehören weiter dem Kunstbundle (D-248)** |
| M24 | — (`profile.json` v1, außerhalb Save+Hash) — **B1 (Schwierigkeit mit Zähnen) löst KEINEN Bump ein und braucht keinen: `difficulty` ist seit M9 gespeicherte, gehashte Weltregel (D-110), und `DIFFICULTY_AI_TRAITS` ist eine reine `const`-Tabelle darüber. Kein Save-Feld, keine Migration, kein Snapshot-Byte, kein Protokollfeld, kein RNG-Draw, keine i18n-Zeile, keine Atlas-Zelle; `SAVE_VERSION` bleibt **34**. Kanonischer Pin `b7e632a7124e67ce`, Korpus `a00868b9911f12d6` und Soak-Fixture `64fec78d6bf0cd5e` bei unveränderten 35 Kommandos und 16 Checkpoints — **nachgeprüft durch Ausführen**, ebenso dass die Normal-Zeile die exakte Identität des Vor-M24-Konkurrenten ist (Vier-Seed-`aiGame`-Summe **7.293.303 EUR auf den Euro**, die Zahl aus D-248). Die Fertig-wenn-Klausel „Hard > Normal bei gleichem Seed" ist **auf 16 von 16 Seeds NICHT erfüllt** und der Grund ist nicht die Tabelle, sondern `START_CAPITAL_CT`: eine Hard-Welt gibt JEDER Firma 250.000 statt 500.000 EUR, drei Konkurrenten sind pro Seed 750.000 EUR ärmer, und die ganze Tabelle wiegt bei konstantem Kapital einstellige Prozent (Terrain-Probe +5,6 %, Chain-Lookahead −4,3 %, Kandidatenzahl/Flotten-Headroom/Tender exakt inert). Gemeldet statt gebandet (D-252)**. **B2 (D-253) löst ebenfalls KEINEN Bump ein und ändert keine gehashte Zeile eines Normal-Weltmodells: `createCompany` bekommt das Eröffnungskapital vom AUFRUFER statt es selbst nachzuschlagen — der Spieler die Zeile seiner Stufe, jeder Konkurrent `AI_START_CAPITAL_CT` (die Normal-Grundlinie) in allen drei Stufen. Kein Save-Feld, keine Migration, kein Snapshot-Byte, kein Protokollfeld, kein RNG-Draw, keine i18n-Zeile, keine Atlas-Zelle; `SAVE_VERSION` bleibt **34**. Kanonischer Pin `b7e632a7124e67ce`, Korpus `a00868b9911f12d6` und Soak-Fixture `64fec78d6bf0cd5e` **nachgeprüft durch Ausführen** und unbewegt — der Normal-Arm ist Seed für Seed bitgleich, Vier-Seed-`aiGame`-Summe weiterhin **7.293.303 EUR auf den Euro**. Nachgemessen bei gleichem Kapital: Easy 29.873.058, Normal 26.215.097, Hard 24.735.440 EUR (Hard +14.214.449 gegen B1); die Fertig-wenn-Ordnung bleibt **auf 11 von 16 Seeds unerfüllt** und wird gemeldet statt gebandet**. **B3 (die Kampagne "Eisenadern", D-254) loest ebenfalls KEINEN Bump ein und braucht keinen: eine Kampagnen-Etappe ist ein gewoehnliches `ShippedScenario`, das durch `newGameOptionsOf` -> `worldParamsFor` -> `World.create` geht, und die Kette selbst (`CAMPAIGN_STAGES`) erreicht die Simulation nie. Kein Save-Feld, keine Migration, kein Snapshot-Byte, kein Protokollfeld, kein RNG-Draw, keine Atlas-Zelle; `SAVE_VERSION` bleibt **34**. Kanonischer Pin `b7e632a7124e67ce`, Korpus `a00868b9911f12d6` und Soak-Fixture `64fec78d6bf0cd5e` bei unveraenderten 35 Kommandos und 16 Checkpoints - **nachgeprueft durch Ausfuehren**, ebenso dass die acht M17-Szenarien bitgleiche Welten erzeugen, obwohl `ScenarioRules` um `startYear` (Pflicht) und `mapgen` (optional) gewachsen ist: `World`s Konstruktor schrieb `?? START_YEAR` und `?? DEFAULT_MAP_GEN_KNOBS` schon vorher, `SCENARIO_WORLD_CLAIMS` ist unveraendert (53 Tests gruen). Zwoelf neue Determinismus-Fixtures (`tests/determinism/campaign.spec.ts`), die erstmals 1850er, 1880er und 1920er Welten je ein Spieljahr spielen und paarweise verschieden hashen. **Der Rest aus D-250 ist gemessen statt beschrieben**: jede Etappe traegt eine Konkurrenz-Sonde ueber sechs Spieljahre - sechs Etappen liefern solo aus (bei zwei hinzugefuegten Konkurrenten null Fahrzeuge, auf der 1880er Etappe 4 und 3 Stationen ohne jede Flotte), sechs liefern mit Konkurrenten aus, deren Seeds durch Spielen ausgewaehlt wurden. Haupt-Bundle 1.041.208 -> **1.048.431 B**, Budget 1.048.000 -> **1.058.000 B** mit der Aufteilung daneben gemessen (+2.123 B i18n, +5.100 B Oberflaeche; die zwoelf Definitionen liegen in einem eigenen 21.360-B-Chunk)** **B4 (Profil, Achievements, KI-Persoenlichkeit und KI-News, D-255) loest ebenfalls KEINEN Bump ein und braucht keinen: `profile.json` ist eine EIGENE versionierte Kette (`PROFILE_VERSION` 1, eigene Migrationen, main-thread-seitig unter `src/platform`), ausserhalb von Save und Hash - `SAVE_VERSION` erfaehrt nie davon, was ein Audit ueber `src/sim` und ueber `save/format.ts` festhaelt. Kein Save-Feld, keine Migration, kein Snapshot-Byte, kein RNG-Zug, keine Atlas-Zelle; `SAVE_VERSION` bleibt **34**. EIN neues Protokollfeld auf dem MARKER-Kanal (`CompanyMarker.personality`, -1 fuer den Spieler; kein Stride-Byte, E-05/Fehlerkatalog 37) und **vier neue News-Zeilen** (`news.aiLineOpened` / `aiLineClosed` / `aiBankrupt` / `leagueMove`). Der News-Log ist gehashter Weltzustand, also war eine Pin-Bewegung zu erwarten - **kanonischer Pin `b7e632a7124e67ce`, Korpus `a00868b9911f12d6` und Soak-Fixture `64fec78d6bf0cd5e` bei unveraenderten 35 Kommandos und 16 Checkpoints stehen trotzdem unbewegt, nachgeprueft durch Ausfuehren**, und der Grund ist gemessen statt erhofft: die beiden ersten Welten haben ueberhaupt keine Konkurrenten, und auf Seed 4711 eroeffnet unter dem D-229-Boden keine Konkurrenzfirma je eine Linie, wird keine abgewickelt und kreuzt sich keine Rangfolge. Die Ranglistenzeile ERFINDET keinen Zustand - sie liest `valueHistory` einen Eintrag frueher (D-180), weshalb sie ohne Save-Bump existieren darf (Z4). `npm run test:balance:full` gruen bei 136 Tests mit jedem Band auf dem Euro (aiGame-Sweep 7.293.303 EUR). Haupt-Bundle 1.048.431 -> **1.069.575 B**, Budget 1.058.000 -> **1.080.000 B** mit der Aufteilung daneben gemessen (+11.093 B die sechsundneunzig i18n-Schluessel in zwei Sprachen - 41 Achievements tragen Titel UND Bedingung -, +10.051 B der Mechanismus)**. **B5 (D-256) loest ebenfalls KEINEN Bump ein und braucht keinen: der Bundle aendert unter `src/` nur zwei Briefings in `src/scenarios/campaign.ts`. Kein Save-Feld, keine Migration, kein gehashtes Byte, kein Snapshot-Byte, kein Protokollfeld, kein RNG-Zug, keine Atlas-Zelle, keine i18n-Zeile; `SAVE_VERSION` bleibt **34**, und kanonischer Pin `b7e632a7124e67ce`, Korpus `a00868b9911f12d6` sowie Soak-Fixture `64fec78d6bf0cd5e` bei unveraenderten 35 Kommandos sind **nachgeprueft durch Ausfuehren** und unbewegt. Inhalt: das Instrument, auf dem eine Schwierigkeitsstufe ueberhaupt messbar ist (Linien-Ueberleben statt Firmenwert, `tests/balance/aiLineSurvival.ts`, sechzehn Abnahme-Seeds - die Fertig-wenn-Anmerkung traegt beide Zahlensaetze), die korrigierte Etappe-11-Briefingzahl samt neuem `CorridorClaim.nearest`/`shortestChain`-Waechter, der Solo-Satz in Etappe 06 samt Waechter in beide Richtungen, und der Kampagnen-Medaillenlauf als Artefakt (`tests/determinism/campaignMedal.spec.ts`, gemessen 9,4 s). Das Haupt-Bundle ist unberuehrt (kein `src/ui`-, `src/render`- oder `src/sim`-Modul geaendert). **B6 (D-257) loest ebenfalls KEINEN Bump ein und braucht keinen: die Schwierigkeit ist seit D-110 gespeicherte, gehashte Weltregel, und dieser Bundle nimmt der `DIFFICULTY_AI_TRAITS`-Tabelle eine SPALTE weg, statt ein Feld hinzuzufuegen. Kein Save-Feld, keine Migration, kein gehashtes Byte, kein Snapshot-Byte, kein Protokollfeld, kein RNG-Zug, keine i18n-Zeile, keine Atlas-Zelle; `SAVE_VERSION` bleibt **34**, und kanonischer Pin `b7e632a7124e67ce`, Korpus `a00868b9911f12d6` sowie Soak-Fixture `64fec78d6bf0cd5e` bei unveraenderten 35 Kommandos und 16 Checkpoints sind **nachgeprueft durch Ausfuehren** und unbewegt - weil die NORMAL-Zeile unangetastet bleibt und die gepinnten Welten Normal-Welten sind. Inhalt: der Chain-Lookahead verlaesst die Stufentabelle und wird `AI_CHAIN_LOOKAHEAD` = 1 fuer alle drei Stufen (er war der einzige Knopf, der alle drei Zeilen trennte, und er ist in beiden Richtungen anti-korreliert zum Firmenwert UND zum expositionskontrollierten Linien-Ueberleben), die Terrain-Sonde traegt die Stufe, und die Fertig-wenn-Ordnung ist ueber sechzehn Seeds auf BEIDEN Instrumenten neu gemessen und als nicht erreichbar GESCHLOSSEN: Firmenwert 26.535.840 / 26.215.097 / 25.998.151 EUR (Hard 216.946 unter Normal), Linien-Ueberleben 50,0 / 48,6 / 51,4 % (Hards Vorsprung 28 Promille gegen 29 Promille je Linie, Expositionskontrolle 42,9 / 42,9 / 40,9 %). Easy 29.873.058 -> 26.535.840 und Hard 24.735.440 -> 25.998.151 EUR, Normal auf den Cent unveraendert. Eine einzige gepinnte Kampagnen-Sonde bewegt sich (Etappe 12, arktisch, Hard: 4/6, 4/12, 2/6 statt 4/0, 2/6, 0/0) und ihr Briefing zitiert die neuen Zahlen in beiden Sprachen | — | +0,00 ms (nichts im Tick-Pfad; die Probe kostet gemessen p50 0,109 ms je geprüftem Kandidaten im KI-Bauzyklus gegen die 1-ms-Klausel) | 0 |
| M25 | — (Protokoll-Bump Envelope-Felder; kein World-Bump) — **B1 (Undo/Redo, D-258) löst KEINEN Bump ein und braucht keinen, und das ist die Entscheidung des Bundles und kein Nebenprodukt: der Session-Ring ist Zustand des AUSSTELLERS, nicht der Welt. Der Inverse-Patch reist IM Command (`ApplyPatch` = 48) — die Zellen mit dem Wert auf BEIDEN Seiten, die Kontext-Zellen der berührten Kacheln, und der auf den Cent historisch verbuchte Betrag (D-092) —, also ist das Ergebnis eine reine Funktion aus (Welt, Command) und ein aus einem Checkpoint nachsimuliertes Replay erreicht denselben Zustand ohne den Ring, der ihn erzeugt hat. Ein payload-freies `Undo`, das im Sim einen Ring poppt, wäre genau der Z4-Bruch, den Fehlerkatalog 23 beschreibt. Kein Save-Feld, keine Migration, kein Snapshot-Byte, kein RNG-Draw, keine Atlas-Zelle; `SAVE_VERSION` bleibt **34**. Kanonischer Pin `b7e632a7124e67ce`, Korpus `a00868b9911f12d6` und Soak-Fixture `64fec78d6bf0cd5e` bei unveränderten 35 Kommandos und 16 Checkpoints — **nachgeprüft durch Ausführen**. Die Fertig-wenn-Klausel ist erfüllt und als Test formuliert: zwölf undoable Kinds einzeln auf `hashWorld` PLUS Layer-Fingerprint (inkl. der abgeleiteten `oceanMask`/`landmassId`, die der Hash nicht deckt), darunter eine Terraform-Kaskade, die Nachbarecken zog, und ein ungültig gewordenes Undo, das GANZ mit i18n-Meldung abgewiesen wird und die Welt byte-identisch lässt; dazu zwei Welten, von denen die eine 50 Bau-Kommandos plus 50 Undos ausführt und die andere nichts — Log 100 gegen 0, Welt-Hash identisch. Haupt-Chunk 1.069.575 → **1.071.641 B** gegen unveränderte 1.080.000 B, Restluft 8.359 B (davon +1.557 B die sechzehn i18n-Zeilen, per Löschen gemessen). Tick-Zeile unberührt: aufgezeichnet wird ausschließlich beim Bau-Command des SPIELERS in einer lebenden Sitzung, nie in einem Tick, nie für einen Konkurrenten und nie in einem Headless-Lauf. **B2 (Klang-Identitaet, D-259) loest ebenfalls KEINEN Bump ein und braucht keinen: Klang ist Z1s Pixelseite, und dieser Bundle aendert unter `src/sim` NULL Byte. Kein Save-Feld, keine Migration, kein gehashtes Byte, kein Snapshot-Byte, kein Protokollfeld, kein RNG-Draw, keine Atlas-Zelle, keine i18n-Zeile; `SAVE_VERSION` bleibt **34** und alle drei Pins stehen unveraendert (`b7e632a7124e67ce` / `a00868b9911f12d6` / `64fec78d6bf0cd5e` bei 35 Kommandos). Die Fertig-wenn-Haelfte des Klangs ist als Test formuliert: fuenf Antriebsklassen mit paarweise verschiedenen Grundtoenen, die Dampfpfeife gegen SPEC.md 18 (Saegezahn-Partialtoene, EIN Rauschzweig, EINE Huellkurve) am Graphen geprueft, zehn gleichzeitige Abfahrten = EINE Pfeife (falsifiziert: mit Fenster 0 liest der Test `expected 10 to be 1`), `PannerNode` mit Distanzmodell an jedem klingenden Knoten samt Klemmung am Bildschirmrand, Master-Kompressor hinter der Summe - und ein Audio-Glob ueber den Git-Index OHNE Allowlist. Haupt-Chunk 1.071.641 -> **1.070.428 B**, also 1.213 B WENIGER als vorher: der erste Build lag mit 1.081.891 B um 1.891 B ueber dem Budget, woraufhin die Engine hinter den dynamischen Import gewandert ist, den die Erst-Geste ohnehin impliziert (11.723 B eigener Chunk, `src/audio/cues.ts` als import-freies Blatt nach dem `save/version.ts`-Muster). Budget unveraendert 1.080.000 B, Restluft 9.572 B**. **B3 (Accessibility, i18n-Skalierung und der Web-Kanal, D-260) loest ebenfalls KEINEN Bump ein: Plural-Regeln, OS-Locale-Erkennung, Schraffur, Reduced-Motion, Roving-Focus und Key-Rebinding sind Settings (D-110), OPFS und der COOP/COEP-Shim sind Plattform - beides Z1s Pixelseite, und auch dieser Bundle aendert unter `src/sim` NULL Byte. Kein Save-Feld, keine Migration, kein gehashtes Byte, kein Snapshot-Byte, kein Protokollfeld, kein RNG-Draw, keine Atlas-Zelle; `SAVE_VERSION` bleibt **34** und alle drei Pins stehen unveraendert (`b7e632a7124e67ce` / `a00868b9911f12d6` / `64fec78d6bf0cd5e` bei 35 Kommandos und 16 Checkpoints), nachgeprueft durch Ausfuehren. Die Fertig-wenn-Haelften dieses Bundles sind als Tests formuliert: Plural-Regeln in BEIDEN Sprachen (Kategorie aus `Intl.PluralRules`, plus zwei Katalog-Waechter gegen eine tote Plural-Zeile), ein Rebinding, das einen Konflikt ABLEHNT und die kollidierende Aktion benennt (zwei Aktionen auf einer Taste sind per Konstruktion unmoeglich, auch fuer eine handgeschriebene Datei), ein OPFS-Backend, das einen Save schreibt, wiederliest und nach einem simulierten Browser-Neustart - Modul-Registry verworfen, nur das Origin-Dateisystem bleibt - immer noch findet, und der AUSGELIEFERTE Shim, dessen Fetch-Handler mit einem falschen Event gefahren wird und dessen Antwort beide Header traegt. **Nicht verifizierbar in einer Headless-Suite und deshalb benannt statt behauptet: dass ein echter Browser danach wirklich `crossOriginIsolated` meldet** - der Beweis waere `dist/` von einem Host ohne beide Header zu servieren, den einen Reload zu sehen und `crossOriginIsolated === true` plus `typeof SharedArrayBuffer !== 'undefined'` zu lesen; die harte SAB-Pflicht bleibt und kein Single-Thread-Pfad ist entstanden. Haupt-Chunk 1.070.428 -> **1.082.561 B**, Budget 1.080.000 -> **1.094.000 B** mit der per Loeschen gemessenen Aufteilung (+2.077 B i18n, +10.056 B Mechanik)** **B4 (Release-Automation, Multiplayer-Groundwork, E-17-Linie, D-261) loest ebenfalls KEINEN Bump ein und braucht keinen - und bucht als einziger dieses Meilensteins den PROTOKOLL-Bump, den die Kopfzeile dieser Zelle seit M25 ankuendigt: `PROTOCOL_VERSION` 1 -> **2** im import-freien Blatt `src/shared/netProtocol.ts` (das `save/version.ts`-Muster, D-192). Er versioniert den NACHRICHTEN-Vertrag, nicht die Welt und nicht das Shared-Buffer-Layout. Kein Save-Feld, keine Migration, kein gehashtes Byte, kein Snapshot-Byte, kein RNG-Draw, keine Atlas-Zelle, keine i18n-Zeile; `SAVE_VERSION` bleibt **34** und alle drei Pins stehen unveraendert (`b7e632a7124e67ce` / `a00868b9911f12d6` / `64fec78d6bf0cd5e` bei 35 Kommandos und 16 Checkpoints), nachgeprueft durch Ausfuehren. **Die zwei reservierten Envelope-Felder (`checksum?`, `sessionId?`) kosten NULL Byte, weil dieser Build keines schreibt** - drei Tests halten das: ein Quell-Walk ueber jede Datei, die `CommandEnvelope` kennt; ein echter Save, dessen zurueckgelesenes Log den SCHLUESSEL nicht hat (nicht `undefined`, gar nicht); und ein fremdes Log, das sie traegt und unveraendert durch Encoder und Parser kommt. **Die Fertig-wenn-Haelfte der Release-Automation ist als Test formuliert, soweit sie ohne Runner pruefbar ist**: `release.yml` auf `v*`, Web-Build + MSI + NSIS + die zur Build-Zeit aus dem Abhaengigkeitsbaum GENERIERTE Attributions-Datei (23 Laufzeit-Pakete, ausschliesslich permissiv; der Generator VERWEIGERT ein Paket ohne Lizenzfeld), beide Haelften des Signatur-Gates samt SmartScreen-Warnung, und ein DRAFT-Release, weil Veroeffentlichen eine Owner-Entscheidung ist. **Die Tick-Budget-Zeile bleibt +0,00 ms und die ~0,1-ms-Schaetzung dieser Zelle wird korrigiert statt still erfuellt**: gemessen auf dem 1500-Fahrzeug-Fixture kostet `hashWorldTick` **6,28 ms** (Median von zwoelf), gegen `hashWorldLive` 6,50 und `hashWorld` 280,6 ms - also 63x die Schaetzung und ~4,3x den ganzen Tick. Das Flag ist AUS und der Digest wird im SCHEDULER nach `step` genommen, ausserhalb `tick()`, also zahlt der ausgelieferte Worker dafuer einen Null-Vergleich je Tick. Haupt-Chunk 1.082.561 -> **1.083.108 B (+547)** gegen unveraenderte 1.094.000 B, Restluft 10.892 B** | — | +0,00 ms (Digest gemessen 6,28 ms je Tick, nur bei Flag - D-261) | 0 |
...
| M24 | 2026-08-13 | **1,580 / 1,420 / 1,568 ms p50** und **3,438 / 2,942 / 3,101 ms p99** (drei Abschlusslaeufe nach allen vier Bundles; max 24,293 / 22,566 / 25,150 ms ueber je 6 500 Ticks) | **+0,18 / -0,32 / -0,16 ms p99** gegen die Grundlinie 1,45 / 3,26 - zwei der drei liegen DARUNTER, der dritte im dokumentierten +-0,7-ms-Laufrauschen; die Budget-Zeile +0,00 ms ist gehalten. Was der Meilenstein dem Tick wirklich hinzufuegt, ist eine Zeile im JAHRES-Hook (`reportLeagueMovement`, liest `valueHistory` und legt nichts an) und zwei Kanten im KI-Entscheidungszyklus, der je Firma alle 400 Ticks laeuft; die Terrain-Sonde aus B1 kostet gemessen p50 0,109 ms je geprueftem Kandidaten und liegt strukturell hinter denselben Intervallen. **Die Referenzflotte hat keine Konkurrenten**, also ist der ganze KI- und News-Pfad auf ihr inert - das steht hier, statt eine Zahl zu erfinden, die das Fixture nicht hergibt (D-201s Haltung) | D-252 - D-255 |
| M24 B5 (D-256) | 2026-08-13 | **keine Tick-Messung, und das ist die ehrliche Angabe** - der Bundle aendert unter `src/` genau zwei Briefing-Zeichenketten in `src/scenarios/campaign.ts` und legt sonst nur Tests an; nichts davon liegt in `tick()` oder unter ihm, also gaebe eine Messung das Laufrauschen dieser Maschine wieder und nicht den Bundle (D-225s Haltung) | +0,00 ms per Konstruktion; die M24-Zeile darueber bleibt die Abnahmezahl des Meilensteins | D-256 |
| M24 B6 (D-257) | 2026-08-13 | **keine Tick-Messung, und das ist die ehrliche Angabe** - der Bundle nimmt der Stufentabelle eine Spalte weg und ersetzt einen Feldzugriff durch eine Konstante; beides liegt im KI-Entscheidungszyklus, der je Firma alle 400 Ticks laeuft, und nichts davon in `tick()` oder unter ihm. Die Referenzflotte hat ausserdem keine Konkurrenten, also waere jede Zahl das Laufrauschen dieser Maschine und nicht der Bundle (D-201s/D-225s Haltung). Gemessen wurde stattdessen, was der Bundle wirklich bewegt: achtundvierzig Vierteljahrhunderte auf zwei Instrumenten, **456,2 s** fuer die ganze Datei | +0,00 ms per Konstruktion; die M24-Zeile oben bleibt die Abnahmezahl des Meilensteins | D-257 |
| M25 B2 (D-259) | 2026-08-13 | **keine Tick-Messung, und das ist die ehrliche Angabe** - der Bundle aendert unter `src/sim` NULL Byte; die Zonen-Zaehlung laeuft auf dem 120-ms-Audio-Timer der Oberflaeche und die Engine auf dem Hauptthread, also waere jede Zahl das Laufrauschen dieser Maschine und nicht der Bundle (D-201s/D-257s Haltung). Gemessen wurde stattdessen, was der Bundle wirklich bewegt: Haupt-Chunk 1.071.641 -> **1.070.428 B** (1.213 B WENIGER, ganze Klang-Identitaet in einem 11.723-B-Chunk hinter dem dynamischen Import) | +0,00 ms per Konstruktion | D-259 |
| M25 B3 (D-260) | 2026-08-13 | **keine Tick-Messung, und das ist die ehrliche Angabe** - der Bundle aendert unter `src/sim` NULL Byte, und seine beiden Render-Zutaten (die Schraffur der Auslastungskarte, das Einfrieren von Partikeln und Wasserzeile) sitzen in Overlays, die auf ein EREIGNIS neu bauen (D-177/D-186) und nicht je Frame. Jede Zahl waere das Laufrauschen dieser Maschine und nicht der Bundle (D-201s/D-257s/D-259s Haltung). Gemessen wurde stattdessen, was der Bundle wirklich bewegt: Haupt-Chunk 1.070.428 -> **1.082.561 B (+12.133)**, davon +2.077 B die neuen Katalogzeilen in zwei Sprachen (per Loeschen gemessen: 1.080.484 B) und +10.056 B die Mechanik; Budget 1.080.000 -> **1.094.000 B** mit dieser Messung daneben (D-192) | +0,00 ms per Konstruktion | D-260 |
| M25 B4 (D-261) | 2026-08-13 | **keine Tick-Messung fuer den Bundle selbst, und das ist die ehrliche Angabe** - der ausgelieferte Pfad bekommt genau einen Null-Vergleich je Tick im Scheduler; jede Zahl waere das Laufrauschen dieser Maschine (D-201s/D-257s/D-259s/D-260s Haltung). **Gemessen wurde stattdessen das Instrument selbst, weil E-16 dafuer eine Zusage ohne Grundlinie gemacht hat (Fehlerkatalog 36)**: auf dem 1500-Fahrzeug-Fixture, Median von zwoelf Aufrufen, kostet `hashWorldTick` **6,281 ms**, `hashWorldLive` 6,502 ms und `hashWorld` 280,627 ms. Also **63x die ~0,1-ms-Schaetzung** und ~4,3x den ganzen Tick - und der Ledger-Zweig, den E-16 als Ersparnis benennt, ist **3,4 % des Walks**; was dominiert, sind die Fahrzeug-PFADE. Die Zusage wird korrigiert statt still erfuellt, das Flag bleibt AUS, und die Budget-Zeile ist +0,00 ms per Konstruktion. Haupt-Chunk 1.082.561 -> **1.083.108 B (+547)** gegen unveraenderte 1.094.000 B, Restluft 10.892 B | +0,00 ms per Konstruktion | D-261 |
| M25 | 2026-08-13 | **1,944 / 1,469 / 1,459 ms p50** und **3,269 / 3,026 / 3,163 ms p99** (drei Abschlusslaeufe nach allen vier Bundles; max 23,128 / 21,633 / 28,446 ms ueber je 6 500 Ticks) | **+0,01 / -0,23 / -0,10 ms p99** gegen die Grundlinie 1,45 / 3,26 - zwei der drei liegen DARUNTER, der dritte 0,01 ms darueber; die Budget-Zeile +0,00 ms ist gehalten. Der erste p50-Wert ist der Lauf unmittelbar nach einem `npm run build` auf derselben Maschine und liegt im dokumentierten +-0,7-ms-Laufrauschen. **Was der Meilenstein am Tick geaendert hat, ist nichts**: B1 zeichnet nur beim Bau-Command des SPIELERS auf, B2 und B3 aendern unter `src/sim` null Byte, und B4 legt einen Null-Vergleich je Tick hinter ein Flag, das aus ist | D-258 bis D-261 |
...
| balance | M10 | jedes Szenario 2× mit Hash-Assertion (ab M16 verbindlich); Referenzläufe mit allen Regeln „aus" — **M16 B3 eingelöst und ehrlich geteilt: der Zwilling ist gemessen (+186 s CPU, davon 143 s allein die zwei 25-Jahre-KI-Szenarien), deshalb laufen im gewöhnlichen `npm test` die sieben billigen Zwillinge (+43 s) und ALLE neun im `soak`-Job je Push; `tests/balance/determinism.ts` trägt die Messwerte, ein Kopplungstest (`tests/unit/balanceDeterminism.spec.ts`) zählt die Szenarien nicht, sondern zählt sie AUF (D-190)**; **M23 B4 eingelöst: die Klima-Matrix (Szenarien 1–4 je Klima, gebandet, plus der D-118-Kettenlauf je Klima-Set) hängt am SELBEN Schalter — gemessen kostet sie über alle vier Klimata 114 s in ihren beiden Standard-Armen und 175 s mit dem Rau-Wetter-Arm, gegen 132,9 s für die ganze Balance-Suite davor. Deshalb fährt jeder Lauf die ersten ZWEI Klimata (`CLIMATE_SWEEP`, temperiert + arktisch, 57 s) und der `soak`-Job alle vier plus den Wetter-Arm; die Sweep-Grenzen hält derselbe Kopplungstest von außen. Ein dritter Verbraucher EINES Schalters, weil der `soak`-Job EIN Job ist (D-248)**; **M24 B5 eingelöst und gebucht (D-256): `aiDifficulty.spec.ts` fährt seit diesem Bundle die SECHZEHN Abnahme-Seeds (`AI_ACCEPTANCE_SEEDS`) statt vier, also achtundvierzig Vierteljahrhunderte in EINEM Durchgang — gemessen **654,7 s** gegen 194 s, d. h. **+7,7 Minuten je Push** im `soak`-Job, in dem diese Datei allein läuft (`describe.runIf(fullBalanceMode())`). Der Grund ist kein Komfort: die Vier-Seed-Sicht hat der Sechzehn-Seed-Sicht zweimal in drei Bundles widersprochen (D-252, D-253), und eine Ordnungsaussage auf den Seeds, die eine Suite zufällig spielt, ist genau der Fehler, gegen den D-220 geschrieben ist. Der erste Lauf hat den Preis eingespielt: `RefitVehicle|insufficientFunds` tritt auf genau EINEM der sechzehn Seeds auf und fiel sofort durch den Declared-Set-Wächter; **M24 B6 (D-257) hat dieselbe Datei erneut gefahren und die Zahl datiert: 456,2 s am 2026-08-13 gegen 654,7 s bei D-256 - dieselben achtundvierzig Vierteljahrhunderte auf einer ruhigeren Maschine, also der fuer dieses Fixture schon von D-220 aufgeschriebene Laufspread und keine Beschleunigung** |
...
| desktop-smoke | M25 | wöchentlich, Rust gecacht; release.yml auf Tag — **M25 B4 eingelöst und ehrlich geteilt (D-261): `release.yml` existiert und fährt auf `v*` (plus `workflow_dispatch` als vollständiger Trockenlauf) drei Jobs — Web-Build auf ubuntu, MSI+NSIS auf windows, ein DRAFT-Release, das beide einsammelt. **Er fährt die Suite bewusst NICHT noch einmal**: `ci.yml` läuft auf jedem Push, also auch auf dem getaggten Commit, und ein Vierzig-Minuten-Duplikat hätte nur dazu verführt, eines von beiden zu überspringen — `release.spec.ts` hält diese Teilung in BEIDE Richtungen (der Release-Workflow darf `npm run test:soak` nicht enthalten, `ci.yml` muss). Ein wöchentlicher desktop-smoke-Job ist damit NICHT geliefert und wird benannt statt behauptet: der Tauri-Build braucht einen Windows-Runner mit Rust-Toolchain und misst dasselbe wie der Release-Job, nur öfter** |
...
*(Anmerkung nach B1, D-252 — die Klausel wird nicht umgeschrieben, sondern
gemessen beantwortet: die Testbänder je Stufe stehen und sind gemessen, die
ORDNUNG „Hard > Normal bei gleichem Seed" ist auf 16 von 16 Seeds NICHT
erfüllt. Der beherrschende Term einer Stufe ist `START_CAPITAL_CT` — eine
Hard-Welt gibt jeder Firma 250.000 statt 500.000 EUR, und mehr zu geben wäre
genau der von SPEC.md 15 verbotene Ressourcen-Bonus. Bei KONSTANTEM Kapital,
Knopf für Knopf gemessen, wiegt die ganze Tabelle einstellige Prozent. Die
Klausel bleibt offen und gehört einem Balance-Pass, nicht einer
Bewertungsfunktion.)*

*(Anmerkung nach B2, D-253 — der ZWEITE Satz der B1-Anmerkung war falsch, die
Messung nicht. `START_CAPITAL_CT` ist laut eigener Dokumentation das Startkapital
des SPIELERS, wurde aber von `createCompany` jeder Firma der Welt gegeben; drei
Konkurrenten waren auf Hard pro Seed 750.000 EUR ärmer, bevor eine einzige
Bewertung stattfand. Seit B2 bekommt der Spieler die Zeile seiner Stufe und jeder
Konkurrent `AI_START_CAPITAL_CT`, die Normal-Grundlinie, in allen drei Stufen —
**kein Ressourcen-Bonus im Sinne von SPEC.md 15**, weil kein Konkurrent je mehr
hält als ein Spieler, der ein Normal-Spiel beginnt: Hard macht nicht die KI
reicher, sondern den Spieler ärmer, und genau das ist eine Schwierigkeitsstufe.
Nachgemessen über dieselben sechzehn Seeds bei jetzt gleichem Kapital
(24.000.000 EUR je Stufe): Easy 29.873.058, Normal 26.215.097, **Hard
24.735.440 EUR** — Hard ist **+14.214.449 EUR** gegenüber der B1-Messung und
**die Ordnung ist weiterhin NICHT erfüllt**: höher als Normal auf 5 von 16 Seeds,
gleich auf 2, niedriger auf 9, in Summe 5,6 % unter Normal, während Easy 14,0 %
darüber liegt. Der Mechanismus ist der Chain-Lookahead — der einzige Knopf, der
sich über alle drei Zeilen unterscheidet, und in beiden Richtungen
anti-korreliert zum Firmenwert im Jahr 25 (D-252). Der zweite Bilanzterm einer
Stufe, `LOAN_INTEREST_RATE_PER_YEAR`, wurde ablatiert gemessen (+1,8 %, eine
Insolvenz weniger) und bewusst NICHT geändert: er ist ein PREIS, den der Spieler
in derselben Welt zahlt. Die Klausel bleibt offen — jetzt aber aus einem Grund
über die BEWERTUNG, und das ist die einzige Art von Antwort, die SPEC.md 15
zulässt.)*

*(Anmerkung nach B3, D-254 — die Kampagne. Zwölf Etappen 1850→2050 über alle
vier Klimata stehen als Text (`src/scenarios/campaign.ts`): geordnete Liste plus
Unlock-Kanten, ein Wurzelknoten, zwei Verzweigungen, die beide wieder
zusammenlaufen, nichts unerreichbar, kein Zyklus — alle drei als eigene Tests,
plus ein Durchlauf, der die Kette wirklich spielt. Eine Etappe ist ein
gewöhnliches `ShippedScenario` und wird über den M17-`GameEndScreen`
abgeschlossen: gebucht wird ausschließlich bei `GameEnd.Won`, mit der Medaille,
die die Simulation entschieden hat. **Die Klausel „Abschluss persistiert über
Neustart hinweg in `profile.json`" ist NICHT eingelöst** — der Fortschritt lebt
in der Sitzung, `profile.json` v1 ist ein eigener MUSS-Punkt und bekommt seinen
eigenen Bundle; ebenso offen bleiben die ~40 Achievements, die Persönlichkeit im
Firmenprofil und der Kampagnen-Medaillenlauf per M16-Replay. **Der Rest aus
D-250 ist beantwortet statt überspielt**: die sechs Etappen von 1850 und 1880
liefern OHNE Konkurrenten aus und sagen es in ihren Briefings, weil eine Sonde
über sechs Spieljahre misst, dass zwei hinzugefügte Konkurrenten dort null
Fahrzeuge erreichen; die sechs Etappen von 1920 und 1950 liefern mit
Konkurrenten aus, deren Seeds durch Spielen ausgewählt wurden, und ihre
Briefings zitieren die gemessenen Stations- und Fahrzeugzahlen, die
`CAMPAIGN_BRIEFING_FIGURES` an dieselbe Sonde bindet.)*

*(Anmerkung nach B4, D-255 — die drei MUSS-Punkte, die B3 offen gelassen hat.
**`profile.json` v1** liegt unter `src/platform` mit eigener Version, eigener
Migrationskette und eigener Datei; `SAVE_VERSION` erfaehrt nie davon, und zwei
Audits halten das fest. Drei Regeln, Ende zu Ende getestet: alles, was kein
Profil ist, liest sich als leeres Profil und das Spiel startet; ein defekter
Block kostet nur sich selbst; ein Profil aus einer NEUEREN Version wird gelesen
und danach eingefroren — `writeProfile` gibt `false` zurueck und die Datei bleibt
byteweise stehen, samt des Blocks, den dieser Build nicht lesen kann.
Geschrieben wird ueber ein Abonnement der drei Store-Felder, damit keine
Buchungsstelle es vergessen kann. **Einundvierzig Achievements** als Daten (35
aus News-Schluesseln, 6 aus M17-Zielen), jeder genannte News-Schluessel gegen
die `'news.*'`-Literale aus `src/sim` geprueft, und die Null-Sim-Beteiligung ist
grep-beweisbar: `achievements`, `ACHIEVEMENT` und `Achievement` kommen unter
`src/sim` nicht vor. **Die KI-Persoenlichkeit** steht in der Konkurrenzliste —
`PERSONALITY_KEYS` war seit M8 exportiert und uebersetzt und wurde von keiner
Datei je benutzt —, und **vier News-Zeilen** geben der Konkurrenz ein Leben
(Linieneroeffnung, Stilllegung, Insolvenz, Jahres-Ranglistenbewegung, alle vier
per `postOnce` an der Kante, an der das Ereignis wirklich passiert). Damit sind
von der Fertig-wenn-Klausel offen: **der Kampagnen-Medaillenlauf per M16-Replay**
und die **Ordnung „Hard > Normal"** aus D-252/D-253 — beides steht als benannter
Rest und nicht als stille Luecke.)*

*(Anmerkung nach B5, D-256 — **die Klausel maß das falsche Ding, und das ist
jetzt gemessen statt behauptet.** ALT: „ein Hard-KI-Lauf … erreicht messbar
höheren FIRMENWERT als Normal". Was das misst, ist eine Bilanz im Jahr 25 —
und die kann eine Linie, die nie gebaut wurde, nicht von einer unterscheiden,
die gestorben ist (D-225). Gemessen bei GLEICHEM Konkurrenzkapital (D-253) über
die sechzehn Abnahme-Seeds: Easy 29.873.058, Normal 26.215.097, **Hard
24.735.440 EUR** — Hard **5,6 % UNTER** Normal, höher auf **5 von 16** Seeds,
und die Stufe mit dem FLACHSTEN Chain-Lookahead (Easy) liegt oben. NEU: das
Instrument misst den GEGNER statt sein Konto — **Linien-Überleben**, d. h. die
am Jahr 25 noch stehenden Linien als Anteil aller je eröffneten, gemessen von
einem reinen Beobachter (`tests/balance/aiLineSurvival.ts`, gegengeprüft gegen
die akzeptierten `CreateLine`-Kommandos aus D-220s Outcome-Sink) über dieselben
sechzehn Seeds: eröffnet 61/35/32, am Jahr 25 stehend 28/17/16, **Anteil 45,9 %
/ 48,6 % / 50,0 %** — monoton in dem einen Knopf, der sich über alle drei Zeilen
unterscheidet und in D-252 NICHT als inert gemessen wurde (Chain-Lookahead
0/1/2; Kandidatenzahl und Flotten-Headroom unterscheiden sich ebenfalls über
alle drei Zeilen und sind exakt inert, die Terrain-Sonde trennt nur Normal von
Hard). Je Seed: Hard über Normal auf 3, gleich auf 10, DARUNTER auf genau 1
(Seed 4714), unbestimmt auf 2 — gegen 5/2/9 beim Firmenwert.

**Die Klausel ist damit auch auf dem NEUEN Instrument nicht geschlossen, und
genau das ist der Befund** — sie wird mit beiden Zahlensätzen gemeldet statt
grün gerechnet, denn die Ordnung auf dem rohen Anteil überlebt weder ihre eigene
AUFLÖSUNG noch ihre eigene KONTROLLE. **Auflösung**: bei 61/35/32 eröffneten
Linien ist EINE Linie 16 / 29 / 31 Promille wert, und die gemessenen Abstände
sind 27 (Easy→Normal) und 14 (Normal→Hard) Promille — beide unter einer Linie.
Wäre EINE der zweiunddreißig Hard-Linien geschlossen worden statt stehen
geblieben, läse Hard 15/32 = 46,9 % und läge UNTER Normal. Dieselbe Datei bandet
je Stufe deshalb ±100 Promille und begründet es mit genau dieser Rechnung; eine
Ordnung, die schmaler ist als ihr eigenes Band-Argument, ist keine Evidenz.
**Kontrolle**: eine Hard-Firma eröffnet ihre Linien im Mittel im Spieljahr 10,60
gegen Easys 7,82, und der rohe Anteil zählt eine im Jahr 24 eröffnete Linie als
Überlebende. Bei GLEICHER Expositionszeit (12,5 Spieljahre Sterberisiko) kippt
die Ordnung: **45,0 % / 42,9 % / 36,8 %**, und die mittlere Lebensdauer einer
geschlossenen Linie trägt gar keine Ordnung (3,23 / 2,70 / 2,88 Jahre). Was ein
tieferer Chain-Test nachweisbar kauft, sind also WENIGER und SPÄTERE Linien,
nicht haltbarere — `aiDifficulty.spec.ts` druckt die Ordnung und pinnt sie als
Messwert von heute, behauptet aber in demselben Atemzug den Confound und die
Auflösung, und an `DIFFICULTY_AI_TRAITS` wurde nichts gedreht, um eine Zahl grün
zu bekommen (D-197). Das Instrument bleibt trotzdem der Gewinn dieses Bundles:
es ist die erste Größe, auf der die Frage überhaupt gestellt werden kann, und es
ist der Regressionsnetz-Band je Stufe. Jede andere geprüfte Kandidatengröße — bemannte Firmen 17/12/10,
Konkurrenzpräsenz 28/17/16 Linien, 168/102/90 Fahrzeuge, 104/63/55 Stationen,
Alter der ältesten überlebenden Linie — ordnet Easy nach oben, weil Easy am
meisten baut. Der Preis steht in 6.3: sechzehn Seeds statt vier, achtundvierzig
Vierteljahrhunderte, gemessen 654,7 s gegen 194 s - und er hat sich im ersten
Lauf bezahlt gemacht, weil `RefitVehicle|insufficientFunds` (nur auf Seed 2718,
x4 gegen 8 akzeptierte) durch D-220s Declared-Set-Waechter fiel und jetzt mit
Messung deklariert ist. **Der Kampagnen-Medaillenlauf ist eingelöst**
(`tests/determinism/campaignMedal.spec.ts`: Etappe 08 wirklich gespielt, Medaille
von der Simulation entschieden, `.ironreplay` verifiziert, Zielblock aus
Checkpoint UND Genesis feldweise reproduziert). Offen bleibt damit von M24 genau
EINE Klausel — die Ordnung „Hard > Normal" —, gemeldet mit beiden Zahlensätzen
und dem Instrument, auf dem sie messbar wäre, statt grün gerechnet; dazu zwei
benannte Reste: die Etappe wird nicht GEWONNEN — ihr zweites Ziel ist
ein Frachtgeschäft —, und dabei ist ein `ConnectStations`-Ziel gefunden worden,
das der FAHRPLAN erfüllt statt einer Verbindung (gemessen auf Etappe 06, wo die
Straße `onWater` abgelehnt wird und das Ziel trotzdem in Tick 200 fällt); das
ist eine Zielmaschinen-Änderung mit eigenem Bundle.)*

*(Anmerkung nach B6, D-257 — **die Klausel wird hier GESCHLOSSEN, und zwar als
mit einer reinen Bewertungsaenderung NICHT ERREICHBAR.** Vier Bundles haben
`DIFFICULTY_AI_TRAITS` Knopf fuer Knopf gemessen, zwei unabhaengige Pruefungen
haben dieselben Zahlen bestaetigt, und das Ergebnis ist eindeutig statt
mehrdeutig: DREI Knoepfe sind nachweislich wirkungslos (die Kandidatenzahl
bindet nie — die laengste Kandidatenliste ueber acht Seeds x 25 Jahre x fuenf
Persoenlichkeiten ist FUENF —, der Flotten-Headroom ist auf Hard wirkungslos,
weil der 12.3-Berater bereits den Deckel verlangt, und die 14.4-Ausschreibung
feuert nie, weil sie Fertigware in eine STADT will), EIN Knopf hilft (die
Terrain-Sonde, +326.566 EUR / +5,6 %), und EIN Knopf SCHADET — ausgerechnet der,
der Hard definiert hat: der Chain-Lookahead, +16,9 % bei Tiefe 0 und −4,3 % bei
Tiefe 2, und ueber den D-256-Expositionsschwellen ab 7,5 Spieljahren ist Hard
auf jeder Schwelle die SCHLECHTESTE Stufe.

**Die Tabelle ist danach umgebaut** (D-257): der Chain-Lookahead ist keine
Spalte mehr, sondern die Konstante `AI_CHAIN_LOOKAHEAD` = 1 fuer jede Stufe —
ein Knopf, der einen Konkurrenten schlechter macht, je weiter man ihn dreht, ist
keine Schwierigkeitseinstellung, und Tiefe 0 (die auf dem Geldinstrument am
reichsten misst) wird ausdruecklich NICHT genommen, weil sie D-109s Regel
loescht (D-225 hat den konkreten Tod gemessen) und den NORMAL-Konkurrenten
bewegen wuerde, also Szenario 5, den `aiGame`-Sweep und das Soak-Fixture aus
einem schliessenden Bundle heraus umbanden (Fehlerkatalog 34). **Das ist eine
benannte Abweichung von der MUSS-Liste oben**, die "Chain-Lookahead-Tiefe" als
Spalte nennt; der Mechanismus bleibt und bleibt parametrisiert, nur die STUFE
entscheidet ihn nicht mehr. Die Stufe traegt jetzt der Knopf mit Zaehnen (die
Terrain-Sonde, 0/0/8).

**Nachgemessen ueber dieselben sechzehn Abnahme-Seeds, auf BEIDEN Instrumenten**
(25 Jahre, drei Konkurrenten, gleiches Konkurrenzkapital): Firmenwert Easy
26.535.840, Normal **26.215.097** (auf den Cent unveraendert, die vier
`aiGame`-Seeds darin weiterhin 7.293.303 EUR), Hard 25.998.151 — Hard **216.946
EUR UNTER** Normal, hoeher auf 6 von 16 Seeds, gleich auf 2, niedriger auf 8.
Linien-Ueberleben: eroeffnet 36/35/35, stehend 18/17/18, Anteil **50,0 / 48,6 /
51,4 %** — Hards Vorsprung ist 28 Promille, EINE Linie ist 29 Promille wert, und
die Expositionskontrolle stellt Hard nach hinten (**42,9 / 42,9 / 40,9 %**). Auf
keinem der beiden Instrumente ordnen die ueberlebenden Knoepfe die Stufen.

**Was eine Schwierigkeitsstufe also WIRKLICH aendert, gemessen und hier
festgeschrieben:** das Startkapital des SPIELERS (800.000 / 500.000 / 250.000
EUR, Faktor 3,2) und seinen Kreditzins (4,0 / 4,0 / 6,5 % im Jahr, ablatiert
+1,8 % und eine Insolvenz, D-253) — dazu die ueberlebenden Bewertungsknoepfe,
deren Summe unter der Aufloesung beider Instrumente liegt: die Sonde misst bei
gleicher Chain-Tiefe zu nichts, die Exklusivrechte kosten rund 10.028 EUR, die
Ausschreibung feuert nie, die Kandidatenzahl bindet nie, der Flotten-Headroom
bindet nur auf Easy. **Die Fertig-wenn-Klausel gilt in dieser Form als
beantwortet: gebandet je Stufe (das war immer erfuellt), Ordnung geschlossen als
nicht erreichbar.**

**Was ein spaeterer Pass aendern muesste, damit sie doch wahr wird** (D-257
fuehrt es aus): Normal etwas geben, wogegen es schlechter sein kann — die Sonde
auf 0/4/8, was aber den Standardkonkurrenten bewegt und also Szenario 5, den
Sweep und das Soak-Fixture neu aufnimmt; die Kandidatenliste ueberhaupt lang
genug machen, dass eine Tiefe bindet (heute fuenf); einem Konkurrenten ein
Geschaeft geben, mit dem eine Ausschreibung zu gewinnen ist (D-225s
Anschluss-Leg); und ueber mehr als sechzehn Seeds messen, weil bei 35 Linien je
Stufe eine Linie 29 Promille wiegt. **Das Regressionsband je Stufe bleibt auf
beiden Instrumenten stehen** — plus eine neue, falsifizierbare Schranke, die den
Schluss selbst aussagt (Normal und Hard innerhalb von 2,5 % Firmenwert und
innerhalb von zwei Linien Ueberleben): ein spaeterer Pass, der Hard wirklich
besser macht, wird dort ROT, und das ist der Unterschied zwischen eine Klausel
schliessen und sie fallen lassen.)*
