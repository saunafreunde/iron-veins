...
gzip 277,04 → 281,92), auf der Platte **924 276 B gegen das 930 000-B-Budget**
von `bundleBudget.spec.ts`. Restluft **5 724 B** — ausdrücklich protokolliert:
...
D-192-Muster. Eigene Chunks: Szenario-Katalog 12,56 kB, `GameEndScreen`
2,41 kB (`React.lazy` — der eine Bildschirm, den eine Sitzung höchstens einmal
zeigt), `replay` 197,99 kB, Worker 324,10 kB, CSS 13,43 kB.
...
`tests/unit/goals.spec.ts`), zwei Läufe mit gleichem Seed hashen bit-identisch
...
mit Balance-Band ✓ (`tests/balance/gameScore.spec.ts`, 5 747 Punkte, vier
Viertel 37/26/25/12 %, D-196).
