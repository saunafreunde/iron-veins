...
/**
 * The Normal purse, and the one every competitor opens on whatever the level is
 * (D-253). Written out here so a reader of this file can see that the opening
 * cash is now the CALLER's statement rather than a lookup inside
 * `createCompany`.
 */
const NORMAL_CAPITAL_CT = AI_START_CAPITAL_CT;

describe('company creation', () => {
  it('opens on exactly the capital it was handed', () => {
    expect(createCompany(0, 'A', 0, START_CAPITAL_CT[Difficulty.Easy]!).cashCt).toBe(
      800_000 * CENTS_PER_EURO,
    );
    expect(createCompany(0, 'A', 0, NORMAL_CAPITAL_CT).cashCt).toBe(500_000 * CENTS_PER_EURO);
    expect(createCompany(0, 'A', 0, START_CAPITAL_CT[Difficulty.Hard]!).cashCt).toBe(
      250_000 * CENTS_PER_EURO,
    );
    expect(START_CAPITAL_CT[Difficulty.Normal]).toBe(500_000 * CENTS_PER_EURO);
    // The competitors' purse IS the Normal row, by derivation and not by a
    // second literal - the whole argument for it is that it is the game's own
    // baseline (D-253).
    expect(AI_START_CAPITAL_CT).toBe(START_CAPITAL_CT[Difficulty.Normal]);
  });

  it('starts debt free', () => {
    const company = createCompany(0, 'A', 0, NORMAL_CAPITAL_CT);
...
describe('interest and the financial year', () => {
  ...
  it('charges the hard difficulty more', () => {
    const easy = createCompany(0, 'A', 0, NORMAL_CAPITAL_CT);
    const hard = createCompany(0, 'A', 0, START_CAPITAL_CT[Difficulty.Hard]!);
